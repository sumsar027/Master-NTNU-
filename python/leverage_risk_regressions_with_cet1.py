...
def _to_decimal_ratio(series: pd.Series) -> pd.Series:
...
def prepare_variables(df):
    ...
    # LCR variables (for Model 4A / 4C)
    if "liquidity_coverage_ratio_basel_3" in df.columns:
        df["lcr_ratio"] = _to_decimal_ratio(df["liquidity_coverage_ratio_basel_3"])
        df["lcr_lag1"] = df.groupby("bank_id")["lcr_ratio"].shift(1)
        df["int_unitvar_lcr"] = df["log_unit_var"] * df["lcr_lag1"]

    # CET1 variables (for Model 4B / 4C)
    cet1_col = _select_cet1_column(df)
    df["cet1_ratio"] = _to_decimal_ratio(df[cet1_col])
...
def estimate_model(df, dep_var, indep_vars, model_name):
    ...
    print(f"{'='*70}")
    print(
        f"Sample: {len(df_clean)} observations, "
...
def main():
    ...
    res1 = estimate_model(df, dep_var="log_leverage", indep_vars="log_unit_var", model_name="Model_1_Leverage")
    all_results.append(res1)

    # MODEL 2: Assets on VaR
    res2 = estimate_model(
        df, dep_var="log_total_assets", indep_vars="log_var_99_level", model_name="Model_2_Assets"
    )
    ...
    # MODEL 4B: Leverage with CET1 Interaction
    res4b = estimate_model(
        df,
        dep_var="log_leverage",
        indep_vars=["log_unit_var", "cet1_lag1", "int_unitvar_cet1"],
        model_name="Model_4B_CET1",
    )
    all_results.append(res4b)

    # MODEL 4C: Leverage with LCR + CET1 Interactions
    res4c = estimate_model(
        df,
        dep_var="log_leverage",
        indep_vars=[
            "log_unit_var",
        ...
        model_name="Model_4C_LCR_CET1",
    )
    all_results.append(res4c)
