...
df = df[
    ["bank_id", "period_end_date", "total_assets", "common_equity_total"]
].copy()
...
df["asset_growth"] = (
    100 * np.log(df["total_assets"]).groupby(df["bank_id"]).diff()
)

df["leverage_growth"] = (
    100 * np.log(df["leverage"]).groupby(df["bank_id"]).diff()
)
